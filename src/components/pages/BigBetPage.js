...
class BigBetPagejs extends Component {
  ...
  killBet(x) {
    const stackId = this.contracts["BetSwap"].methods.killBig.cacheSend(x, {
  ...
  getbetHistoryArray() {
    ...
    const contractweb3b = new web3b.eth.Contract(BettingContract.abi, BettingContract.address);
    var eventdata = [];
    var takes = {};
    var eventdata2 = [];
    var takes2 = {};
...
takes2[element.returnValues.contractHash] = this.contracts["BetSwap"
    ...
    makeBigBet() {
      const stackId = this.contracts["BetSwap"]
  ...
  radioTeamPickTake(betamt0, hash0, odds0) {
    this.setState({teamTake: true, contractID: hash0, betAmount: betamt0,
      decOddsOff: odds0})
  }
...
let minBet = 0;
    if (this.minBetKey in this.props.contracts["BetSwap"].minBet) {
      minBet = this.props.contracts["BetSwap"].minBet[this.minBetKey].value
    }
